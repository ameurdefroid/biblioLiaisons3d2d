...
// Boite englobante 
triple coinBas = (0)*b0.x + (0)*b0.y + (-e/2)*b0.z ;
triple coinHaut = (dec+b+lJ+JK+KL)*b0.x + (2)*b0.y + (e/2)*b0.z ;
...
for(int i=0; i < n ; ++i) {
  save();
   ...
   // Relation LES
   real theta80 = (-8+i*pas)/360*2*pi ; // angle de sortie droit
   ...
   /* pour placer le point B correctement 
      ...
      le premier ou le denrier point du tableau en fonction du sens de création du cercle
   ...
   liaisonRotuleDoigt(J, b0.x, b1.y, CEC2, CEC3, rapide=true) ;
   liaisonRotuleDoigt(K, -b0.x, b1.z, CEC2, CEC1, rapide=true) ;
